    ...
    public static class Orbwalking
    {
        ...
        public static float GetRealAutoAttackRange(AttackableUnit target)
        {
            ...
            if (target.IsValidTarget())
            {
                ...
                if (aiBase != null && _Player.ChampionName == "Caitlyn")
                {
        ...
        public static bool CanAttack()
        {
            ...
            return Core.GameTickCount >= LastAATick + _Player.AttackDelay * 1000 && Attack;
        }
        ...
        public static bool CanMove(float extraWindup)
        {
            ...
            return (Core.GameTickCount >= LastAATick + _Player.AttackCastDelay * 1000 + extraWindup);
        }
            ...
            bool randomizeMinDistance = true)
        {
            ...
            var angle = 0f;
            var currentPath = _Player.Path;
            /*
            if (currentPath.Count() > 1 && cur > 100)
            {
        ...
        public class Orbwalker
        {
            ...
            {
                _config = MainMenu.AddMenu("Orbwalker 2", "samorbwalkerlolol");
                /* Drawings submenu */
                var drawings = _config.AddSubMenu("Drawings", "drawings");
                /*
                drawings.AddItem(
                ...
                /* Misc options */
                var misc = _config.AddSubMenu("Misc", "Misc");
                misc.Add("HoldPosRadius", new Slider("Hold Position Radius", 0, 0, 250));

                //misc.AddItem(
                //    new MenuItem("HoldPosRadius", "Hold Position Radius").SetShared().SetValue(new Slider(0, 0, 250)));
                misc.Add("PriorizeFarm", new CheckBox("Prioritize farm over harass"));
                ...
                misc.Add("FocusMinionsOverTurrets", new CheckBox("Focus minions over objectives", false));
                ...
                /* Missile check */
                _config.Add("MissileCheck", new CheckBox("Use Missile Check", true));

                //_config.AddItem(new MenuItem("MissileCheck", "Use Missile Check").SetShared().SetValue(true));

                /* Delay sliders */
                _config.Add("ExtraWindup", new Slider("Extra windup time", 35, 0, 250));
                _config.Add("FarmDelay", new Slider("Farm delay", 0, 0, 250));
                ...
                _config.Add("Flee", new KeyBind("Flee", false, KeyBind.BindTypes.HoldActive, 'Z'));

                _config.Add("LastHit", new KeyBind("Last Hit", false, KeyBind.BindTypes.HoldActive, 'X'));
                
                _config.Add("Farm", new KeyBind("Harass", false, KeyBind.BindTypes.HoldActive, 'C'));

                _config.Add("LaneClear", new KeyBind("Lane Clear", false, KeyBind.BindTypes.HoldActive, 'V'));
                ...
                _config.Add("FastHarass", new KeyBind("FastHarass", false, KeyBind.BindTypes.HoldActive, 'Y'));
            ...
            {
                get { return _config["FarmDelay"].Cast<Slider>().CurrentValue; }
            }
            ...
            {
                get { return _config["MissileCheck"].Cast<CheckBox>().CurrentValue; }
            }
            ...
            public OrbwalkingMode ActiveMode
            {
                get
                {
                    ...
                    if (_config["Orbwalk"].Cast<KeyBind>().CurrentValue)
                    {
                        return OrbwalkingMode.Combo;
                    }

                    if (_config["StillCombo"].Cast<KeyBind>().CurrentValue)
                    {
                        return OrbwalkingMode.Combo;
                    }

                    if (_config["LaneClear"].Cast<KeyBind>().CurrentValue)
                    {
                        return OrbwalkingMode.LaneClear;
                    }

                    if (_config["Farm"].Cast<KeyBind>().CurrentValue)
                    {
                        return OrbwalkingMode.Mixed;
                    }

                    if (_config["LastHit"].Cast<KeyBind>().CurrentValue)
                    {
                        return OrbwalkingMode.LastHit;
                    }

                    if (_config["Flee"].Cast<KeyBind>().CurrentValue)
                    {
                        return OrbwalkingMode.Flee;
                    }

                    if (_config["FastHarass"].Cast<KeyBind>().CurrentValue)
                    {
                        return OrbwalkingMode.FastHarass;
                    }

                    if (_config["Burst"].Cast<KeyBind>().CurrentValue)
                    {
                        return OrbwalkingMode.Burst;
                    }

                    /*if (_config[CustomModeName] != null && _config[CustomModeName].Cast<KeyBind>().CurrentValue)
                    {
            ...
            {
                return EntityManager.MinionsAndMonsters.Minions.Any(
                    minion =>
                                minion.IsValidTarget() && minion.Team != GameObjectTeam.Neutral &&
                                InAutoAttackRange(minion) &&
                                Prediction.Health.GetPrediction(minion, (int)(((_Player.AttackDelay * 1000) * LaneClearWaitTimeMod)+FarmDelay)) <=
            ...
            private void GameOnOnGameUpdate(EventArgs args)
            {
                try
                {
                    ...
                    Move = !_config["StillCombo"].Cast<KeyBind>().CurrentValue;
                    ...
                    Orbwalk(
                        ...
                        _config["ExtraWindup"].Cast<Slider>().CurrentValue,
                        Math.Max(_config["HoldPosRadius"].Cast<Slider>().CurrentValue, 30));
                }
                catch (Exception e)
                {
                    Console.WriteLine(e);
            ...
            private void DrawingOnOnDraw(EventArgs args)
            {
                ...
                /*if (_config["AACircle"].Cast<Circle>().Active)
                {
                    Render.Circle.DrawCircle(
                        _Player.Position, GetRealAutoAttackRange(null) + 65,
                        _config["AACircle"].Cast<Circle>().Color,
                        _config["AALineWidth"].Cast<Slider>().Value);
                }

                if (_config["AACircle2"].Cast<Circle>().Active)
                {
                        ...
                        HeroManager.Enemies.FindAll(target => target.IsValidTarget(1175)))
                    {
                        Render.Circle.DrawCircle(
                            ...
                            _config["AACircle2"].Cast<Circle>().Color,
                            _config["AALineWidth"].Cast<Slider>().Value);
                    }
                }

                if (_config["HoldZone"].Cast<Circle>().Active)
                {
                    Render.Circle.DrawCircle(
                        _Player.Position, _config["HoldPosRadius"].Cast<Slider>().Value,
                        _config["HoldZone"].Cast<Circle>().Color,
                        _config["AALineWidth"].Cast<Slider>().Value, true);
                }
                _config["FocusMinionsOverTurrets").Permashow(_config["FocusMinionsOverTurrets"].Cast<KeyBind>().CurrentValue)*/;
            }
        }
    }
}